...
exports.overrideActorProperties = overrideActorProperties;
...
function createActorSourceProperty(displayOptions) {
    return {
        ...actorSourceProperty,
        displayOptions,
    };
}
function overrideActorProperties(properties) {
    const result = [];
...
async function listActors(searchTerm) {
    const actorSource = this.getNodeParameter('actorSource', 'recentlyUsed');
    const mapToN8nSelectOption = (actor) => {
    ...
    const { data: { items: storeActors }, } = await genericFunctions_1.apiRequestAllItems.call(this, {
        method: 'GET',
        uri: '/v2/store',
        qs: {
            limit: 200,
...
//# sourceMappingURL=actorResourceLocator.js.map